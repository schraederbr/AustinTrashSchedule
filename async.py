...
async def sampleAddresses(session, file_name, file_name_2, count=20):
...
async def main():
    async with aiohttp.ClientSession() as session:
        await sampleAddresses(session, "Addresses.csv", "sample.csv", 10000)
    splitByTrashDate('sample.csv')

def splitByTrashDate(file_name = 'sample.csv'):
    ...
    # Explode the list of dates into separate rows
    df = df.explode('TRASH_DATE')

    # Group by `TRASH_DATE` and save each group to a separate CSV file
    for trash_date, group in df.groupby('TRASH_DATE'):
        group.to_csv(f'samples/trash_date_{trash_date}.csv', index=False)
